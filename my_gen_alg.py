...
class GA:
    ...
    def __init__(self,expression_data,pop_size,num_gen,init_num_removed,mutation_probability,crossover_probability,max_removed) -> None:
    ...
    def fitness_funct(self):
        ...
        self.fitness = self.get_p_value() *  (1 - num_removed/self.max_removed)* 0.1 * np.multiply(num_removed < self.max_removed,1)
    ...
    def fitness_funct2(self):
        num_not_removed = np.sum(self.population,axis = 1)
        num_removed = self.ind_length - num_not_removed
        self.fitness = self.get_p_value() *  np.log(2 - num_removed/self.max_removed) * np.multiply(num_removed < self.max_removed,1)
        
    def fitness_funct3(self):
        num_not_removed = np.sum(self.population,axis = 1)
        num_removed = self.ind_length - num_not_removed
        self.fitness = self.get_p_value() 
        self.fitness = self.fitness + (self.fitness * (1 - num_removed/self.max_removed)) * np.multiply(num_removed < self.max_removed,1)
    ...
    @staticmethod
    def mutate2(mut_prob,max_removed,offspring):
    ...
    def mutation_func2(self):
        mut = partial(GA.mutate2, self.mutation_probability,self.max_removed)
        with mp.Pool() as pool2:
            res = np.array(map(mut, self.population))
            self.population = res
...
ga = GA(expression_data,2500,130,3,0.55,0.2,200)  
ga()
